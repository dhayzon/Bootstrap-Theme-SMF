...
function template_show_list($list_id = null)
{
	...
	if (!empty($cur_list['title']))
		echo '
			<div  class="alert alert-warning" role="alert">				 
					<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> ', $cur_list['title'], '				 
			</div>';
	...
	if (!empty($cur_list['items_per_page']) || isset($cur_list['additional_rows']['bottom_of_list']))
	{
		...
		if (!empty($cur_list['items_per_page']))
			echo '
				<div class="pull-left">
					<div class="smfPagination">', $txt['pages'], ': ', $cur_list['page_index'], '</div>
				</div>';
	...
	echo '
			<table class="table" width="', !empty($cur_list['width']) ? $cur_list['width'] : '100%', '">';
			...
			foreach ($list_menu['links'] as $link)
			{
				if ($link['is_selected'])
					echo '
							...
							<td v class="', $direction == 'top' ? 'mirrortab' : 'maintab', '_active_back">
								<a href="', $link['href'], '">', $link['label'], '</a>
							</td>
							<td class="', $direction == 'top' ? 'mirror' : 'main', 'tab_active_', $last, '">&nbsp;</td>';
				else
					echo '
							<td v class="', $direction == 'top' ? 'mirror' : 'main', 'tab_back">
								<a href="', $link['href'], '">', $link['label'], '</a>
							</td>';
			}
