...
function template_modify_subscription()
{
			...
			<div class="panel-body">
				...
				<div class="content">
					<dl class="dl-horizontal">
						...
						<dd>
							<input type="text" name="name" value="', $context['sub']['name'], '" size="30" class="form-control" />
						</dd>
						<dt>
							', $txt['paid_mod_desc'], ':
						</dt>
					...
					<dl class="dl-horizontal">
						...
						<dd>
							<select class="form-control" name="prim_group" ', !empty($context['disable_groups']) ? 'disabled="disabled"' : '', '>
	...
	echo '
						...
						<dd>
							<input type="text" name="reminder" value="', $context['sub']['reminder'], '" size="6" class="form-control" />
						</dd>
						<dt>
					...
					<div id="fixed_area" ', empty($context['sub']['duration']) || $context['sub']['duration'] == 'fixed' ? '' : 'style="display: none;"', '>
						<fieldset>
							<dl class="dl-horizontal">
								...
								<dd>
									<input type="text" name="cost" value="', empty($context['sub']['cost']['fixed']) ? '0' : $context['sub']['cost']['fixed'], '" size="4" class="form-control" />
								</dd>
								<dt>
									', $txt['paid_mod_span'], ':
								</dt>
								<dd>
									<input type="text" name="span_value" value="', $context['sub']['span']['value'], '" size="4" class="form-control" />
									<select class="form-control" name="span_unit">
	...
	echo '
							...
							<dl class="dl-horizontal">
								...
								<dd>
									<input type="text" name="cost_day" value="', empty($context['sub']['cost']['day']) ? '0' : $context['sub']['cost']['day'], '" size="5" class="form-control" />
								</dd>
								<dt>
									', $txt['paid_per_week'], ':
								</dt>
								<dd>
									<input type="text" name="cost_week" value="', empty($context['sub']['cost']['week']) ? '0' : $context['sub']['cost']['week'], '" size="5" class="form-control" />
								</dd>
								<dt>
									', $txt['paid_per_month'], ':
								</dt>
								<dd>
									<input type="text" name="cost_month" value="', empty($context['sub']['cost']['month']) ? '0' : $context['sub']['cost']['month'], '" size="5" class="form-control" />
								</dd>
								<dt>
									', $txt['paid_per_year'], ':
								</dt>
								<dd>
									<input type="text" name="cost_year" value="', empty($context['sub']['cost']['year']) ? '0' : $context['sub']['cost']['year'], '" size="5" class="form-control" />
								</dd>
							</dl>
						</fieldset>
					</div>
					<div class="righttext">
						<input type="submit" name="save" value="', $txt['paid_settings_save'], '" class="btn btn-default" />
...
function template_modify_user_subscription()
{
	...
	if ($context['action_type'] == 'add')
		echo '
						...
						<dd>
							<input type="text" name="name" id="name_control" value="', $context['sub']['username'], '" size="30" class="form-control" />
						</dd>';
	...
	echo '
					...
					<fieldset>
						...
						<select class="form-control" name="year" id="year" onchange="generateDays(\'\');">';
	...
	echo '
						...
						', $txt['hour'], ': <input type="text" name="hour" value="', $context['sub']['start']['hour'], '" size="2" class="form-control" />
						', $txt['minute'], ': <input type="text" name="minute" value="', $context['sub']['start']['min'], '" size="2" class="form-control" />
					</fieldset>
					<fieldset>
						<legend>', $txt['end_date_and_time'], '</legend>
						<select class="form-control" name="yearend" id="yearend" onchange="generateDays(\'end\');">';
	...
	echo '
						...
						', (isset($txt['calendar_month']) ? $txt['calendar_month'] : $txt['calendar_month']), '&nbsp;
						<select class="form-control" name="monthend" id="monthend" onchange="generateDays(\'end\');">';
	...
	echo '
						...
						', (isset($txt['calendar_day']) ? $txt['calendar_day'] : $txt['calendar_day']), '&nbsp;
						<select class="form-control" name="dayend" id="dayend">';
	...
	echo '
						...
						', $txt['hour'], ': <input type="text" name="hourend" value="', $context['sub']['end']['hour'], '" size="2" class="form-control" />
						', $txt['minute'], ': <input type="text" name="minuteend" value="', $context['sub']['end']['min'], '" size="2" class="form-control" />
					</fieldset>
	...
	if (!empty($context['pending_payments']))
	{
		...
		foreach ($context['pending_payments'] as $id => $payment)
		{
			echo '
					<li class="reset">
						', $payment['desc'], '
						<span class="pull-left"><a href="', $scripturl, '?action=admin;area=paidsubscribe;sa=modifyuser;lid=', $context['log_id'], ';pending=', $id, ';accept">', $txt['pending_payments_accept'], '</a></span>
						<span class="pull-right"><a href="', $scripturl, '?action=admin;area=paidsubscribe;sa=modifyuser;lid=', $context['log_id'], ';pending=', $id, ';remove">', $txt['pending_payments_remove'], '</a></span>
					</li>';
		}
	...
	else
	{
		...
		foreach ($context['subscriptions'] as $id => $subscription)
		{
			...
			if ($context['user']['is_owner'])
			{
				...
				{
					echo '
					<select class="form-control" name="cur[', $subscription['id'], ']">';
				...
				echo '
					<br />
					<input type="submit" name="sub_id[', $subscription['id'], ']" value="', $txt['paid_order'], '" class="btn btn-default" />';
			}
			else
				echo '
		...
		<div class="information">
			...
			<thead>
				<tr>
					...
					<th class="text-center">', $txt['paid_status'], '</th>
					<th class="text-center">', $txt['start_date'], '</th>
					<th class="text-center">', $txt['end_date'], '</th>
				</tr>
			</thead>
			<tbody>';
...
function template_choose_payment()
{
	...
	foreach ($context['gateways'] as $gateway)
	{
		...
		echo '
						<br /><input type="submit" value="', $gateway['submit'], '" class="btn btn-default" />
					</form>
			</div>
					</div>';
	}
