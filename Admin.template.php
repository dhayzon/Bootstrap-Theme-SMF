...
function template_not_done()
{
	...
	echo '
				<form action="', $scripturl, $context['continue_get_data'], '" method="post" accept-charset="', $context['character_set'], '" style="margin: 0;" name="autoSubmit" id="autoSubmit">
					<div style="margin: 1ex; text-align: right;"><input type="submit" name="cont" value="', $txt['not_done_continue'], '" class="btn btn-default" /></div>
...
function template_show_settings()
{
	...
	foreach ($context['config_vars'] as $config_var)
	{
		...
		if (is_array($config_var))
		{
			...
			else
			{
				...
				{
					echo '
							<select class="form-control" name="', $config_var['name'], '" id="', $config_var['name'], '" ', $javascript, $disabled, (!empty($config_var['multiple']) ? ' multiple="multiple"' : ''), '>';
	...
	if (empty($context['settings_save_dont_show']))
		echo '
					<hr class="hrcolor clear" />
					<div class="righttext">
						<input type="submit" value="', $txt['save'], '"', (!empty($context['save_disabled']) ? ' disabled="disabled"' : ''), (!empty($context['settings_save_onclick']) ? ' onclick="' . $context['settings_save_onclick'] . '"' : ''), ' class="btn btn-default" />
					</div>';
...
function template_edit_profile_field()
{
				...
				<div class="content">
					<fieldset>
						...
						<dl class="dl-horizontal">
							...
							<dd>
								<select class="form-control" name="profile_area">
					...
					<fieldset>
						...
						<dl class="dl-horizontal">
							...
							<dd>
								<select class="form-control" name="field_type" id="field_type" onchange="updateInputBoxes();">
	...
	echo '
					...
					<fieldset>
						...
						<dl class="dl-horizontal">
							...
							<dd>
								<select class="form-control" name="mask" id="mask" onchange="updateInputBoxes();">
							...
							<dd>
								<select class="form-control" name="private" id="private" onchange="updateInputBoxes();" style="width: 100%">
	...
	if ($context['fid'])
		echo '
						<input type="submit" name="delete" value="', $txt['delete'], '" onclick="return confirm(\'', $txt['custom_edit_delete_sure'], '\');" class="btn btn-default" />';
...
function template_core_features()
{
	...
	echo '
			<div class="righttext">
				...
				<input type="submit" value="', $txt['save'], '" name="save" class="btn btn-default" />
			</div>
		</form>
	</div>
	<br class="clear" />';
...
function template_download_language()
{
	...
	echo '
			<div class="righttext padding">
				<input type="hidden" name="', $context['session_var'], '" value="', $context['session_id'], '" />
				<input type="submit" name="do_install" value="', $txt['add_language_smf_install'], '" class="btn btn-default" />
			</div>
		</form>
	</div>
	<br class="clear" />';
...
function template_modify_language_entries()
{
	...
	echo '
			...
			<div class="windowbg">
				...
				<div class="content">
					...
					<div class="righttext">
						<input type="hidden" name="', $context['session_var'], '" value="', $context['session_id'], '" />
						<input type="submit" name="save_main" value="', $txt['save'], '"', $context['lang_file_not_writable_message'] || !empty($context['file_entries']) ? ' disabled="disabled"' : '', ' class="btn btn-default" />';

	// English can't be deleted.
	if ($context['lang_id'] != 'english')
		echo '
						<input type="submit" name="delete_main" value="', $txt['delete'], '"', $context['lang_file_not_writable_message'] || !empty($context['file_entries']) ? ' disabled="disabled"' : '', ' onclick="confirm(\'', $txt['languages_delete_confirm'], '\');" class="btn btn-default" />';
	...
	if (!empty($context['file_entries']))
	{
		...
		echo '
					...
					<input type="submit" name="save_entries" value="', $txt['save'], '"', !empty($context['entries_not_writable_message']) ? ' disabled="disabled"' : '', ' class="btn btn-default" />';
